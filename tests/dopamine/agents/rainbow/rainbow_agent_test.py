...
class RainbowAgentTest(tf.test.TestCase):
  ...
  def _create_test_agent(self, sess):
    ...
    class MockRainbowNetwork(tf.keras.Model):
      ...
      def call(self, state):
        ...
        qs = tf.reduce_sum(input_tensor=self.support * probabilities, axis=2)
  ...
  def testShapesAndValues(self):
    with tf.compat.v1.Session() as sess:
      ...
      self.assertEqual(
          self.evaluate(tf.reduce_min(input_tensor=agent._support)), -self._vmax)
      self.assertEqual(self.evaluate(tf.reduce_max(input_tensor=agent._support)), self._vmax)
